...
function IrrigationController(adapter, timeframes = [], zones = []) {
    ...
    this.zones.forEach(function (controller) {
        ...
        controller.zone.isSeedTime.onUpdate(function () {
            self.irrigation.isSeedTime.value = self.zones.some(function (current) {
                return current.zone.isSeedTime.value === true;
            });
        });
    });
}
...
IrrigationController.prototype.isInside = function() {
    if(this.irrigation.isSeedTime.value === true) {
        return true;
    }
